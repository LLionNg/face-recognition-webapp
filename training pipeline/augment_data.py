        ...
        for class_name in tqdm(class_dirs, desc="Processing classes"):
            ...
            for image_file in tqdm(
                ...
                if augmented_images:
                    ...
                    saved = self.save_augmented_images(
                        augmented_images, image_path, train_path
                    )
                    augmented_count += saved
                    total_augmented += saved

            logger.info(
                f"  Class {class_name}: {original_count} original → {augmented_count} augmented"
            )
...
def main():
    parser = argparse.ArgumentParser(
        description="Augment face recognition training data"
    )
    parser.add_argument(
        "--data_dir",
        "-d",
        default="known_people_photos",
    ...
    try:
        ...
        logger.error(
            "Albumentations not installed. Install with: pip install albumentations opencv-python numpy"
        )
        return

    # Initialize augmenter
    augmenter = FaceDataAugmenter(
        augment_factor=args.augment_factor, output_suffix=args.suffix
    )

    # Run augmentation
    augmenter.augment_dataset(data_dir=args.data_dir, preview_mode=args.preview)
