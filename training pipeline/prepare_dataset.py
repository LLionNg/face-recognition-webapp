...
def prepare_dataset(source_dir, dest_dir):
    ...
    for root, dirs, files in os.walk(source_dir):
        ...
        for filename in tqdm(files, desc=f"Cropping faces for {class_name}", leave=False):
            if filename.lower().endswith(('.jpg', '.jpeg', '.png', '.bmp')):
                ...
                try:
                    ...
                    if face_tensor is not None:
                        face_np = (face_tensor.permute(1, 2, 0).detach().cpu().numpy() * 127.5 + 127.5).astype(np.uint8)
