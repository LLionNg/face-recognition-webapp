            ...
            if os.path.exists(target_path) and os.path.isdir(target_path):
                ...
                for img_name in os.listdir(class_dir):
                    if img_name.lower().endswith(
                        (".jpg", ".jpeg", ".png", ".bmp")
                    ) and os.path.isfile(os.path.join(class_dir, img_name)):
                        img_path = os.path.join(class_dir, img_name)
                        self.samples.append((img_path, class_idx))

        logger.info(
            f"Loaded {len(self.samples)} {self.split} samples from {len(classes)} classes"
        )
...
class FaceNetTrainer:
    ...
    def __init__(
        self,
        data_dir,
        model_save_path="facenet_model.pt",
        batch_size=32,
        learning_rate=0.001,
        num_epochs=50,
    ):
        ...
        self.train_dataset = FaceDataset(
            data_dir, transform=self.transform, split="train"
        )
        ...
        if len(self.val_dataset.samples) == 0:
            logger.warning(
                "No validation samples found in Test/ directories! Using 20% of training data for validation."
            )
    ...
    def save_model(self, path=None):
        ...
        with open(mappings_path, "w") as f:
            json.dump(
                {"class_to_idx": self.class_to_idx, "idx_to_class": self.idx_to_class},
                f,
                indent=2,
            )
