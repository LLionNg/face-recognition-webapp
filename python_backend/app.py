...
models_manager = ModelsManager(
    facenet_path="models/best_facenet_model.pt", confidence_threshold=0.5
)
...
async def detect_and_recognize(
    request: ImageRequest, manager: ModelsManager = Depends(get_models_manager)
):
    """
    ...
    try:
        ...
        for i, (face_img, bbox, prob) in enumerate(zip(cropped_faces, bboxes, probs)):
            ...
            logger.info(
                f"Face {i+1} result: {name} (class_id={class_id}, confidence={recognition_confidence:.3f})"
            )
        ...
        for i, face in enumerate(faces):
            logger.info(
                f"  Face {i+1}: {face.name} ({face.recognition_confidence*100:.1f}%)"
            )
