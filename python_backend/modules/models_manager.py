...
class ModelsManager:
    ...
    async def load_models(self):
        ...
        try:
            ...
            self.facenet_model, self.idx_to_class = self.load_facenet_from_checkpoint(
                self.facenet_path
            )
            ...
            if self.idx_to_class:
                self.face_recognizer = FaceRecognizer(
                    self.facenet_model, self.idx_to_class, self.confidence_threshold
                )
                logger.info(
                    f"Face recognizer initialized with {len(self.idx_to_class)} classes"
                )
