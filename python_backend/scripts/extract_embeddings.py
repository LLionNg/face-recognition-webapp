...
def load_facenet_model(model_path):
    ...
    logger.info(
        f"Model loaded with {num_classes} classes (ignored pretrained logits mismatch)"
    )
...
def generate_known_faces_database(
    model_path, photos_dir, output_file="known_faces.json"
):
